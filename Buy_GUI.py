...
def addtoCart(item, id):

    #CPU
    cursor.execute("Select name, price FROM cpu WHERE name ='" + item + "'")
    ...
    #Case
    # cursor.execute("Select name, price FROM case WHERE name = %s", (item,))
    cursor.execute("SELECT name, price FROM computer_store.case WHERE name='" + item + "'")
    records = cursor.fetchall()
    count = cursor.rowcount
    if count == 1:
        for record in records:
...
def viewCart(id):
    ...
    Buy_Button = Button(Button_Frame, text = "Buy this Build", width = 13, command = lambda id=id:buy(id))

...
import os
from Buy_Functions import addtoCart
...
def selectCase():
    ...
    cursor.execute("SELECT name, case_type, side_panel, external_bay, internal_bay, max_gpu_size, max_fan_size, num_front_fan, num_top_fan, num_exhaust_fan, num_fan_included, max_radiator, rgb, price from cases")
    records = cursor.fetchall()
    count = 0
    for record in records:
        if count % 2 == 0:

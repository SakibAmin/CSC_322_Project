...
def create_logged_home(id):
    ...
    def populate(frame, canvas):
        ...
        tk.Button(frame, text = "Browse Catalog", command = lambda id=id: Browse_Catalog(id), width=40, font=('Helvetica', 12)).grid(row=2, column=0)

        #Log Out
        tk.Button(frame, text = "Log_Out", command = lambda id=id: Log_Out(id), width=40, font=('Helvetica', 12)).grid(row=2, column=1)

        #Account Options Menu
        tk.Button(frame, text = "Account", command = lambda id=id: Account_Settings(id), width=40, font=('Helvetica', 12)).grid(row=2, column=2)
        ...
        tk.Button(frame, command=lambda id=id: B_Y_O(id), text="Build your own PC!", borderwidth=0, font=('Helvetica', 12)).grid(row=4, column=1)
    ...
    root = tk.Tk()
    root.title('Home Page: Welcome Customer #' + str(id))
